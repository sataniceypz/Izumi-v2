...
async function serialize(msg, client) {
  ...
  if (msg.message) {
    ...
    try {
      ...
      msg.quoted.isSelf = msg.quoted.sender === msg.decodeJid(client.user.id);
      ...
      msg.quoted.delete = () =>
        client.sendMessage(msg.from, { delete: msg.quoted.key });
    ...
    msg.getQuotedObj = msg.getQuotedMessage = async () => {
      ...
      let q = await store.loadMessage(msg.from, msg.quoted.stanzaId, client);
      return serialize(q, client);
    };
    ...
    client.sendFile = async (jid, path, options = {}) => {
      let type = await client.getFile(path, true);
      ...
      let m;
      try {
        m = await client.sendMessage(jid, message, { ...options });
      } catch (e) {
        console.error(e);
        m = null;
      } finally {
    ...
    msg.adReply = async (text) => {
     ...
     await client.sendMessage(msg.from, { text: text, contextInfo: { externalAdReply: {
     title: "𝐳𝐞𝐭𝐚𝐚𝐚𝐡𝐡 ࿊",
     body: "",
     sourceUrl: "",
     mediaUrl: "",
